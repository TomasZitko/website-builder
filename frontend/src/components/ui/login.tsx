...
import { ArrowRight, Mail, Gem, Lock, Eye, EyeOff, AlertCircle, Loader2 } from "lucide-react";
...
export const LoginComponent = ({ logo = <DefaultLogo />, brandName = "WebChat.ai", onSuccess }: LoginComponentProps) => {
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isLoading || authStep !== 'password' || !isPasswordValid) return;
  ...
  return (
    <div className="bg-background min-h-screen w-screen flex flex-col">
        ...
        <div className={cn("flex w-full flex-1 h-full items-center justify-center bg-card", "relative overflow-hidden")}>
            ...
            <fieldset disabled={isLoading} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                        ...
                        <BlurFade delay={0.25 * 2}><p className="text-sm font-medium text-muted-foreground">Sign in with</p></BlurFade>
                        <BlurFade delay={0.25 * 3}><div className="flex items-center justify-center gap-4 w-full">
                            <GlassButton
                              size="sm"
                              onClick={() => window.location.href = `${import.meta.env.VITE_API_URL || 'http://localhost:4000'}/api/v1/auth/google`}
                            >
                              <GoogleIcon /><span>Google</span>
                            </GlassButton>
                            <GlassButton
                              size="sm"
                              onClick={() => window.location.href = `${import.meta.env.VITE_API_URL || 'http://localhost:4000'}/api/v1/auth/github`}
                            >
                    ...
                    {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        ...
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">Please enter your password to continue</p></BlurFade>
                    </motion.div>}
                </AnimatePresence>

                {error && (
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="w-[300px] p-3 bg-red-500/10 border border-red-500/20 rounded-lg flex items-start gap-2">
                ...
                <form onSubmit={handleFinalSubmit} className="w-[300px] space-y-6">
                     <AnimatePresence>
                        {authStep !== 'password' && <motion.div key="email-field" exit={{ opacity: 0, filter: 'blur(4px)' }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full space-y-6">
                        ...
                        {authStep === "password" && <BlurFade key="password-field" className="w-full space-y-4">
                            <div className="relative w-full">
                                <AnimatePresence>
                                ...
                                <div className="glass-input-wrap w-full"><div className="glass-input">
                                    ...
                                    <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isPasswordValid ? "w-auto pr-1" : "w-0" )}>{isPasswordValid && <GlassButton type="submit" size="icon" aria-label="Sign in" disabled={isLoading}>{isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <ArrowRight className="w-5 h-5" />}</GlassButton>}</div>
                                </div></div>
                            </div>
                            <BlurFade inView delay={0.1}><div className="flex items-center justify-between">
                              <button type="button" onClick={handleGoBack} className="flex items-center gap-1 text-sm text-foreground/70 hover:text-foreground transition-colors"><span>←</span> Go back</button>
                              <button type="button" onClick={() => window.location.href = '/forgot-password'} className="text-sm text-foreground/70 hover:text-foreground transition-colors">Forgot password?</button>
                            </div></BlurFade>
                        </BlurFade>}
                    </AnimatePresence>
                </form>
